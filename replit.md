...
*   **Content Management**: Features detailed content fields for news and events, including image and video management that supports multiple media via URLs or Replit Object Storage uploads, stored as JSONB arrays (`news.images`, `news.videos`). Video URLs support YouTube and Vimeo embeds with automatic conversion, plus external link fallback for other providers. Static page content (About, Contact) is database-driven and multilingual.
