...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/organization-members/:id", optionalAuthenticateToken, async (req, res) => {
    try {
      ...
      const member = await storage.getOrganizationMember(parsedId.data);
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        const memberId = organizationMemberIdSchema.parse(req.params.id);
...
function hasSameOrigin(req: Request): boolean {
  ...
  if (origin) return origin === requestOrigin(req) || sameOriginFetch;

  const referer = req.get("referer");
  if (referer) {
    try {
      return new URL(referer).origin === requestOrigin(req) || sameOriginFetch;
    } catch {
      return sameOriginFetch;
    }
  }
